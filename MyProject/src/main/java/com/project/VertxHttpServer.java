...
public class VertxHttpServer extends AbstractVerticle {
    ...
    public void start() throws Exception {
        ...
        router.route(HttpMethod.POST,"/requestStudent").handler(rc ->{
            ...
            try {
                newWorkshop = objectMapper.readValue(Workshop,Workshape.class);
                if (this.isThisWorkShopeInDatabase(newPerson.getName(),newWorkshop.getId())) {
                    if (pay.equals("2")) {
                        if (!isThisPayInstallment(newWorkshop.getId()))
                            response.end("{\"status\":0}");
                    }
                    newWorkshop = findWorkShopeInDataBase(newWorkshop.getId(),newWorkshop.getNameOfWorkshape());
                    newRequestStudent = new RequestStudent("", (Student) newPerson.findOurType(Type.Student),findPayThisWorkShopInDataBase(newWorkshop.getId()));// bayad tozihat ye chiz ezafeh konam
                    if (newWorkshop.AddToRequestList(newRequestStudent)) {
                        upadateDataBaseWithWorkShop(newWorkshop);
                        response.end("{\"status\":1}");
                    }
                    else
        ...
        router.route(HttpMethod.POST,"/requestStudentCancel").handler(rc -> {
                    ...
                    if (mapLogin.containsKey(token))
                        ...
                        response.end("{\"status\":0}");
                    newWorkshop = findWorkShopeInDataBase(newWorkshop.getId(),newWorkshop.getNameOfWorkshape());
                    if (newWorkshop.deleteStudentFromRequestList(newPerson.getUser()))
                        response.end("{\"status\":1}");
                    else
                        response.end("{\"status\":0}");
                });
    ...
    private void upadateDataBaseWithWorkShop(Workshape newWorkshop) {
    }

    private Pay findPayThisWorkShopInDataBase(String id) {
        return new Pay(10000,true,id);
    }

    private Workshape findWorkShopeInDataBase(String id, String name) {
        return new Workshape(id,name);
    }
